...
public class GRP extends ReactContextBaseJavaModule {
  ...
  public void getRealPathFromURI(String uri, Callback callback) {
    try {
      ...
      callback.invoke(null, realPath);*/
      
      String realPath;
      // SDK < API11
      if (Build.VERSION.SDK_INT < 11) {
          realPath = getRealPathFromURI_BelowAPI11(context, Uri.parse(uri));
      }
      // SDK >= 11 && SDK < 19
      else if (Build.VERSION.SDK_INT < 19) {
          realPath = getRealPathFromURI_API11to18(context, Uri.parse(uri));
      }
      // SDK > 19 (Android 4.4) and up
      else {
          realPath = getRealPathFromURI_API19(context, Uri.parse(uri));
      }
